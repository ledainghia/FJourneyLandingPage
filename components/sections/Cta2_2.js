export default function Cta2_2() {
    return (
        <>
            <section id="cta-2" className="bg-primary-color grey-textured wide-100 cta-section division">
                <div className="container white-color">
                    <div className="row">
                        <div className="col-lg-10 offset-lg-1">
                            <div className="cta-txt text-center">
                                <h3 className="h3-xs">Create better experiences with ShiftKey</h3>
                                <p className="p-md">Feugiat eros, ac tincidunt ligula massa in faucibus orci luctus et ultrices
                                    posuere cubilia curae integer congue leo metus, eu mollis lorem primis in orci integer
                                    metus mollis faucibus
                                </p>
                                <div className="btns-group">
                                    <a href="#contacts-1" className="btn btn-white tra-hover mr-15">Request Free Trial</a>
                                    <a href="#" className="btn btn-tra-white black-hover">Buy For Only $14.99</a>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </section>
        </>
    )
}
